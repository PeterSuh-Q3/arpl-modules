...
MODULE_PARM_DESC(lb_pending_cmds, "Change raid-1 load balancing outstanding threshold."
" Valid Values are 1-128. Default: 4");
...
/* Prototypes */
void	mr_update_load_balance_params(struct megasas_instance *instance, 
...
static MR_SPAN_BLOCK_INFO *MR_LdSpanInfoGet(u32 ld, MR_DRV_RAID_MAP_ALL *map)
{
	return &map->raidMap.ldSpanMap[ld].spanBlock[0];
}

static u8 MR_LdDataArmGet(u32 ld, u32 armIdx, MR_DRV_RAID_MAP_ALL *map)
{
...
u8 MR_TargetIdToLdGet(u32 ldTgtId, MR_DRV_RAID_MAP_ALL *map)
{
	return map->raidMap.ldTgtIdToLd[ldTgtId];
}

static MR_LD_SPAN *MR_LdSpanPtrGet(u32 ld, u32 span, MR_DRV_RAID_MAP_ALL *map)
{
...
static int MR_PopulateDrvRaidMap(struct megasas_instance *instance)
{
	...
	if(instance->supportmax256vd) {
		memcpy(fusion->ld_drv_map[instance->map_id & 1], fusion->ld_map[instance->map_id & 1], 
			fusion->current_map_sz);
		/* New Raid map will not set totalSize, so keep expected value 
 		 * for legacy code in ValidateMapInfo
 		 */
		pDrvRaidMap->totalSize = cpu_to_le32(sizeof(MR_FW_RAID_MAP_EXT));
	} else {
		fw_map_old = (MR_FW_RAID_MAP_ALL*) fusion->ld_map[(instance->map_id & 1)];
		pFwRaidMap = &fw_map_old->raidMap;
		ld_count = (u16)le32_to_cpu(pFwRaidMap->ldCount);
		
		if (ld_count > MAX_LOGICAL_DRIVES) {
			printk("megaraid_sas: LD count exposed in RAID map in not valid\n");
			return 1;
		}
#if VD_EXT_DEBUG
		for (i = 0; i < ld_count; i++) {
			printk("megaraid_sas(%d) : Index 0x%x Target Id 0x%x Seq Num 0x%x Size 0/%llx\n",
				instance->unique_id,
        	                i, fw_map_old->raidMap.ldSpanMap[i].ldRaid.targetId,
		...
		for(i = 0; i < (MAX_RAIDMAP_LOGICAL_DRIVES + MAX_RAIDMAP_VIEWS); i++)
			pDrvRaidMap->ldTgtIdToLd[i] = (u8)pFwRaidMap->ldTgtIdToLd[i];
		...
		for (i = 0; i < ld_count; i++) {
...
#if VD_EXT_DEBUG
	        printk("megaraid_sas(%d) : pFwRaidMap->ldSpanMap[%d].ldRaid.targetId 0x%x "
        	...
        	printk("megaraid_sas(%d) : pDrvRaidMap->ldSpanMap[%d].ldRaid.targetId 0x%x"
			"pDrvRaidMap->ldSpanMap[%d].ldRaid.seqNum 0x%x size 0x%x\n",i, i,
			instance->unique_id, pDrvRaidMap->ldSpanMap[i].ldRaid.targetId, 
			pDrvRaidMap->ldSpanMap[i].ldRaid.seqNum, 
			(u32)pDrvRaidMap->ldSpanMap[i].ldRaid.rowSize);
        	printk("megaraid_sas(%d) : drv raid map all %p raid map %p LD RAID MAP %p/%p\n", 
...
u8 MR_ValidateMapInfo(struct megasas_instance *instance)
{
	...
	BUG_ON(!pDrvRaidMap);
	if(instance->supportmax256vd) 
		expected_size = sizeof (MR_FW_RAID_MAP_EXT);
	else
		expected_size = (sizeof (MR_FW_RAID_MAP) - sizeof(MR_LD_SPAN_MAP) +
				(sizeof(MR_LD_SPAN_MAP) * le16_to_cpu(pDrvRaidMap->ldCount)));

	if (le32_to_cpu(pDrvRaidMap->totalSize) != expected_size) {
		printk(KERN_ERR "megasas: map info structure size 0x%x is not matching with ld count\n",
		       (unsigned int) expected_size);
		printk(KERN_ERR "megasas: span map %x, pDrvRaidMap->totalSize : %x\n", 
	...
	mr_update_load_balance_params(instance, drv_map, lbInfo);
...
u32    MR_GetSpanBlock(u32 ld, u64 row, u64 *span_blk, MR_DRV_RAID_MAP_ALL *map)
{
...
u32 mr_spanset_get_span_block(struct megasas_instance *instance, u32 ld, u64 row, 
		u64 *span_blk, MR_DRV_RAID_MAP_ALL *map)
{
	struct fusion_context *fusion = instance->ctrl_context;
	MR_LD_RAID         *raid = MR_LdRaidGet(ld, map);
	LD_SPAN_SET *span_set;
...
static u64  get_row_from_strip(struct megasas_instance *instance, 
	u32 ld, u64 strip, MR_DRV_RAID_MAP_ALL *map)
{
	struct fusion_context *fusion = instance->ctrl_context;
	MR_LD_RAID	*raid = MR_LdRaidGet(ld, map);
	LD_SPAN_SET 	*span_set;
...
static u64 get_strip_from_row(struct megasas_instance *instance, 
		u32 ld, u64 row, MR_DRV_RAID_MAP_ALL *map)
{
	struct fusion_context *fusion = instance->ctrl_context;
	MR_LD_RAID         *raid = MR_LdRaidGet(ld, map);
	LD_SPAN_SET *span_set;
	...
	if (pd != MR_PD_INVALID)
		...
		*pDevHandle = MR_PD_INVALID; // set dev handle as invalid.
		if ((raid->level >= 5) && 
			...
			physArm = physArm + 1;
			pd = MR_ArPdGet(arRef, physArm, map); // Get Alternate Pd.
			if (pd != MR_PD_INVALID)
				*pDevHandle = MR_PdDevHandleGet(pd, map); // Get dev handle from Pd.
		}
	}
			...
			u8 **raidLUN)
{
	...
	pRAID_Context->timeoutValue    = cpu_to_le16(raid->fpIoTimeoutForLd ? raid->fpIoTimeoutForLd : 
...
void
mr_update_load_balance_params(struct megasas_instance *instance, 
...
static u8 megasas_get_best_arm_pd(struct megasas_instance *instance, PLD_LOAD_BALANCE_INFO lbInfo, 
		struct IO_REQUEST_INFO *io_info)
{
	struct fusion_context *fusion;
	MR_LD_RAID  *raid;
...
#if SPAN_DEBUG
	if(arm != bestArm)
		printk("LSI Debug R1 Load balance occur - span 0x%x arm 0x%x bestArm 0x%x "
